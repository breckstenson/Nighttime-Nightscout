...
#define SCREEN_WIDTH 128 //if you are using a diffent screen, change these to match
...
void getBg() {
  ...
  int bg = ns["bgs"][0]["sgv"]; //get all our variables from the json
  String delta = ns["bgs"][0]["bgdelta"];
  String bgDirection = ns["bgs"][0]["direction"];
  time_t nsNow = ns["status"][0]["now"]; //current time according to ns in unix
  time_t bgTime = ns["bgs"][0]["datetime"]; //time since last reading in unix
  String iob = ns["bgs"][0]["iob"];
  ...
  int nsHour = hour(); //hour of the time given from nightscout
  int nsMin = minute(); //minute of the current time

  int bgHour = hour(bgTime); //hour of the last reading time
  int bgMin = minute(bgTime); //minute of the last reading time
